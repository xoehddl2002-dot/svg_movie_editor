...
const visElems =
    "a,circle,ellipse,foreignObject,g,image,line,path,polygon,polyline,rect,svg,text,F,use,clipPath";
 ...
 */
export const dropXMLInternalSubset = (str: string) => {
 ...
 */
export function encode64(input: string) {
 ...
 */
export function decode64(input: string) {
 ...
 * @returns {string}
 */
export function decodeUTF8(argString: string) {
 ...
 * @returns {string}
 */
export const encodeUTF8 = (argString: string) => {
 ...
 */
export const convertToXMLReferences = (input: string) => {
    let output = "";
    [...input].forEach((ch) => {
 ...
 * @returns {XMLDocument}
 */
export const text2xml = (sXML: string) => {
 ...
 */
export const bboxToObj = ({ x, y, width, height }: SVGRect): BBoxObject => {
 ...
 * @returns {void}
 */
export const walkTree = (elem: Element, cbFn: (elem: Element) => void) => {
    if (elem?.nodeType === 1) {
        cbFn(elem);
        let i = elem.childNodes.length;
        while (i--) {
            walkTree(elem.childNodes.item(i) as Element, cbFn);
        }
    }
};
 ...
 * @returns {void}
 */
export const walkTreePost = (elem: Element, cbFn: (elem: Element) => void) => {
    if (elem?.nodeType === 1) {
        let i = elem.childNodes.length;
        while (i--) {
            walkTree(elem.childNodes.item(i) as Element, cbFn);
 ...
 */
export const getUrlFromAttr = function (attrVal: string) {
    if (attrVal) {
        // url('#somegrad')
 ...
 */
export let getHref = function (elem: Element) {
 ...
 * @returns {void}
 */
export let setHref = function (elem: Element, val: string) {
...
export const getPathBBox = function (path: SVGPathElement) {
    ...
    const getCalc = function (j: number, P1: number[], P2: number[], P3: number[]) {
 ...
 */
export const getBBox = function (elem: any) {
 ...
 */
export const getPathDFromSegments = function (pathSegments: any[]) {
    let d = "";
 ...
 */
export const getPathDFromElement = function (elem: any) {
 ...
 */
export const getExtraAttributesForConvertToPath = function (elem: Element) {
    const attrs: any = {};
 ...
 */
export const convertToPath = (elem: any, attrs: any, svgCanvas: any) => {
    const batchCmd = new svgCanvas.history.BatchCommand(
 ...
 */
function bBoxCanBeOptimizedOverNativeGetBBox(angle: number, hasAMatrixTransform: boolean) {
 ...
 */
const getStrokeOffsetForBBox = (elem: Element) => {
    const sw = elem.getAttribute("stroke-width");
    return !isNaN(Number(sw)) && elem.getAttribute("stroke") !== "none" ? Number(sw) / 2 : 0;
};
 ...
 * @returns {module:utilities.BBoxObject|module:math.TransformedBox|DOMRect} A single bounding box object
 */
export const getStrokedBBox = (elems: any[], addSVGElementsFromJson: any, pathActions: any) => {
 ...
 */
export const getVisibleElements = (parentElement: any) => {
    if (!parentElement) {
 ...
 */
export const getStrokedBBoxDefaultVisible = (elems: any[]) => {
    if (!elems) {
        elems = getVisibleElements(null);
    }
 ...
 */
export const getRotationAngleFromTransformList = (tlist: SVGTransformList | undefined, toRad?: boolean) => {
    if (!tlist) {
        return 0;
 ...
 * @returns {Float} The angle in degrees or radians
 */
export let getRotationAngle = (elem: any, toRad: boolean) => {
    const selected = elem || svgCanvas.getSelectedElements()[0];
 ...
 */
export const getRefElem = (attrVal: string) => {
    const url = getUrlFromAttr(attrVal);
    if (url) {
        return getElement(url.substr(1));
    }
    return null;
};
/**
 ...
 */
export const getFeGaussianBlur = (ele: any) => {
    if (ele?.firstChild?.tagName === "feGaussianBlur") {
        ...
        for (const [_, value] of Object.entries(childrens)) {
            if ((value as any).tagName === "feGaussianBlur") {
                return value;
            }
        }
    }
 ...
 * @returns {void}
 */
export const assignAttributes = (elem: Element, attrs: any, suspendLength?: number, unitCheck?: boolean) => {
    for (const [key, value] of Object.entries(attrs)) {
        ...
        if (ns) {
            elem.setAttributeNS(ns, key, value as string);
        } else if (!unitCheck) {
 ...
 * @returns {void}
 */
export const cleanupElement = (element: Element) => {
    const defaults: any = {
        "fill-opacity": 1,
        "stop-opacity": 1,
        opacity: 1,
 ...
 * @returns {void}
 */
export const preventClickDefault = (img: Element) => {
    $click(img, (e: any) => {
        e.preventDefault();
    });
};
...
export const insertChildAtIndex = (parent: Element, child: string, index = 0) => {
...
export const $qq = (sel: string) => document.querySelector(sel);
export const $qa = (sel: string) => [...document.querySelectorAll(sel)];
export const $click = (element: Element, handler: any) => {
